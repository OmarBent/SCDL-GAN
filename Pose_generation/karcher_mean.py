...
def karcher_mean(data):
    ...
    while np.linalg.norm(MuV, 'fro') > eps:
        ...
        MuV = (1/n) * MuV
        mean = centeredscaled(metric.exp(tangent_vec=eps*MuV.reshape(1, n*dim), base_point=mean.reshape(1, n*dim)).reshape(n, dim))[0]
